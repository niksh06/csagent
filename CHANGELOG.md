...
- **Complete day as a file: `tparser-day-slice` cron builtin (I-173)** — the daily digest is a curated top-N and always will be: ~40 items out of ~550 posts, 7% of the day, because 550 posts do not fit a Telegram message and a model asked to read them all would burn an hour and still drop most. This adds the other half — a deterministic render of the **whole** window, no model in the loop, delivered as a ~250 KB Markdown attachment beside the digest. Coverage is 100% by construction precisely because nothing in it decides what is interesting. Bucketing uses TParser's own `category` field (`AI`, `AI Security`, `InfoSec`, `Programming`, `DevSecOps`, plus `General News`/`General Tech` kept whole as "вне тем" — a third of the day lands there and silently dropping it would be a lie about coverage). Three things were measured rather than assumed: (1) **no dedup** — exact-text folding collapses nothing (each channel gets its own LLM summary) and token-set near-dup at Jaccard 0.45 merges 2% while still missing the obvious clusters, so the slice lists every post and says so; (2) the head of each section is a **rank** cut (top fifth, floor 3, cap 25), because TParser calibrates priority per category and a fixed ≥0.80 selected 26 of 42 AISec posts and 0 of 57 Programming ones; (3) posts with `analysis_status: none` are pulled into their own «Не разобрано» section — all 64 on the measured day carry the *default* category `General News` and the *default* priority `0.3152`, so filing them as low-priority off-topic news would report a verdict nobody made. Uses `by-date-range` with offset paging (never `recent-live`, whose `after_rowid` is a forward live-tail cursor), caps at 20 pages and **says in the header** when it hits the cap instead of shipping a partial day as a complete one. The API token is read from TParser's own `.env` and only ever travels in an `Authorization` header — never a command line, which is how it previously leaked into transcripts.
